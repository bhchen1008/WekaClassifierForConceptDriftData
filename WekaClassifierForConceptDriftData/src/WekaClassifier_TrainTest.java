...
public class WekaClassifier_TrainTest {
   ...
   */
  public WekaClassifier_TrainTest() {
    super();
  }
...
//          + m_Filter.getClass().getName() + "\n");
  ...
  public static String usage() {
    return
        "\nusage:\n  " + WekaClassifier_TrainTest.class.getName() 
        + "  <trainingfile>\n"
        + "  <testingfile>\n"
        + "  <Output_Result_Path>\n"
        + "  <CLASSIFIER_name>\n\n"
  ...
  public static void main(String[] args) throws Exception {
    WekaClassifier_TrainTest         demo;

    if (args.length < 4) {
      System.out.println(WekaClassifier_TrainTest.usage());
      System.exit(1);
    }
...
//	System.out.println(eval.toClassDetailsString());
    ...
    if(matchFound) {
    	...
    	if(matchFound){
    		...
    		String testFileTmp = testPath.split("/")[inputFileLen-1];
    		tmpLen = testFileTmp.split("\\.").length;
    		String testFile = testFileTmp.split("\\.")[0];
    		if(tmpLen>2){
    			for(int i=1;i<tmpLen-1;i++){
    				testFile += "." + testFileTmp.split("\\.")[i]; 
    			}    			
    		}
