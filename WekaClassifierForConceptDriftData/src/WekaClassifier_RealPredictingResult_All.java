...
public class WekaClassifier_RealPredictingResult_All {
...
//    Instances unlabeled = new Instances(new BufferedReader(new FileReader(args[1])));
    ...
    for(int i=0;i<testAll.numInstances();i++){
    	//測試Accuracy用,放在for迴圈裏頭每次重新計算
    	if(i==1498)
    		System.out.println();
        eval = new Evaluation(train);
        
        testlabeled.add(testAll.instance(i));
		testUnlabeled.add(testAll.instance(i));
...
//    		fileName = testPath.split("/")[inputFileLen-1].split("\\.")[0];
				filePath = outputPath + "/" + "AccuracySet_"+classifier+"/"
						+ fileName + "_AllTime_Accuracy.csv";
